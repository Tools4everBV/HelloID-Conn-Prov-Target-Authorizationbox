...
try {
    ...
    if ($actionContext.DryRun -eq $true) {
        Write-Information "[DryRun] Would delete user"
    }
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        ...
        if($SamAccountName){
            Invoke-Command -Session $s -ScriptBlock {$DisableNavUser = Set-NAVServerUser -ServerInstance $using:NavServerInstanceName -Tenant $using:tenant -WindowsAccount $using:SamAccountName -State Disabled -ErrorAction Inquire -Verbose -Force}
        }
    }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not delete Authorizationbox account. Error: $($_.Exception.Message)"
